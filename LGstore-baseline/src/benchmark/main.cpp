...
int main(int argc, char* argv[]) {
    std::string* opt = read_option_from_file(argc, "option2.txt");
    auto flags = parse_flags(argc, opt);
    ...
    if (keys_file_type == "binary") {
        if (load_binary_data(keys, total_num_keys, keys_file_path) == false) {
            std::cerr << "keys file not exist" << std::endl;
            return 1;
        }
    } else if (keys_file_type == "text") {
        if (load_text_data(keys, total_num_keys, keys_file_path) == false) {
            std::cerr << "keys file not exist" << std::endl;
            return 1;
        }
    } else {
    ...
    //std::unordered_map<KEY_TYPE, PAYLOAD_TYPE> m;
    //for (int i = 0; i < init_num_keys; i++) {
    //    m.insert(std::pair<KEY_TYPE, PAYLOAD_TYPE>(values[i].first, values[i].second));
    //}
    ...
    traverse_all_leaf_to_file(&index, "init_leaf.txt");
    ...
    // 在一个batch中，执行 batch_size * insert_frac 次的插入，其余的执行查询
    ...
    while (true) {
        ...
        // 执行 num_actual_inserts 次插入
        for (; i < num_keys_after_batch; i++) {
            index.insert(keys[i], static_cast<PAYLOAD_TYPE>(gen_payload()));
            //m.insert(std::pair<int, int>(keys[i], static_cast<PAYLOAD_TYPE>(gen_payload())));
        }
    ...
    traverse_all_leaf_to_file(&index);
    delete[] keys;
    delete[] values;
}
