...
class AlexDataNode_Secondary : public AlexNode<T, P> {
 public:
  typedef std::pair<T, P> V;
  typedef AlexDataNode_Secondary<T, P, Compare, Alloc, allow_duplicates> self_type;
  ...
  explicit AlexDataNode_Secondary(const Compare& comp = Compare(), const Alloc& alloc = Alloc())
                        : AlexNode<T, P>(0, true), key_less_(comp), allocator_(alloc) {}

  AlexDataNode_Secondary(short level, int max_data_node_slots, const Compare& comp = Compare(), const Alloc& alloc = Alloc())
               : AlexNode<T, P>(level, true), key_less_(comp), allocator_(alloc), max_slots_(max_data_node_slots) {}

  ~AlexDataNode_Secondary() {
#if ALEX_DATA_NODE_SEP_ARRAYS
  ...
  AlexDataNode_Secondary(const self_type& other)
      : AlexNode<T, P>(other),
  ...
  std::pair<int, int> insert(const T& key) {
    ...
    if (insertion_position < data_capacity_ && !check_exists(insertion_position)) {
...
#endif
      insert_element_at(key, insertion_position);
    } else {
      // 这个位置非空
#ifdef PRINT_PROCESS
      ...
      insertion_position = insert_using_shifts(key, insertion_position);
    }
...
#endif
    if (new_bitmap == nullptr || new_key_slots == nullptr) {
  ...
  void insert_element_at(const T& key, int pos) {
#if ALEX_DATA_NODE_SEP_ARRAYS
    key_slots_[pos] = key;
#else
  ...
  int insert_using_shifts(const T& key, int pos) {
