...
// 这一层所有的 fanout node 记录在 used_fanout_tree_nodes 中
// 返回这一层的 cost
...
double compute_level(const std::pair<T, P> values[], int num_keys,
  ...
  for (int i = 0; i < fanout; i++) {
    // values 的左右边界
    // values[left_boundary] ~ values[right_boundary - 1] 是这个 fanout 管理的数据
    // 根据 right_boundary_value 计算 right_boundary，right_boundary_value 每个 fanout 平均分配
    left_boundary = right_boundary;
    right_boundary = i == fanout - 1 ? num_keys : static_cast<int>(std::lower_bound(values, values + num_keys, ((i + 1) - b) / a,
    ...
    while (right_boundary < num_keys && static_cast<int>(a * values[right_boundary].first + b) <= i) {
      right_boundary++;
    }
    // 这个 fanout 为空
    ...
    // 为这个 fanout 管理的数据计算模型和 cost
    LinearModel<T> model;
    AlexDataNode<T, P>::build_model(values + left_boundary, right_boundary - left_boundary, &model, approximate_model_computation);
    DataNodeStats stats;
    ...
    used_fanout_tree_nodes.push_back({level, i, node_cost, left_boundary, right_boundary, false,
         stats.num_search_iterations, stats.num_shifts, model.a_, model.b_,
         right_boundary - left_boundary});
  }
  double traversal_cost = kNodeLookupsWeight + (kModelSizeWeight * fanout * (sizeof(AlexDataNode<T, P>) + sizeof(void*)) * total_keys / num_keys);
  cost += traversal_cost;
  return cost;
}
...
#ifdef PRINT_PROCESS
    std::cout << "construct fanout tree, level = " << fanout_tree_level << ", fanout = " << fanout << " finish" <<std::endl;
#endif
    fanout_costs.push_back(cost);
    // 连续两层 fanout nodes 的 cost 递增
    if (fanout_costs.size() >= 3 && fanout_costs[fanout_costs.size() - 1] > fanout_costs[fanout_costs.size() - 2] 
...
#ifdef PRINT_PROCESS
  std::cout << "construct fanout tree finish" << std::endl;
#endif
  ...
  while (true) {
    ...
    for (FTNode& tree_node : fanout_tree[fanout_tree_level - 1]) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        AlexDataNode<T, P>::build_model(values + left, right - left, &node_models[i], approximate_model_computation);
