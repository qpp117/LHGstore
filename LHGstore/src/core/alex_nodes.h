...
// std::allocator<std::pair<T, P>>> 用于为 pair<T, P> 类型变量分配空间，但不进行对象的构造
// std::allocator对象包括: allocate方法，用于分配空间
//                        deallocate方法，用于释放空间
//                        construct(p, args) 方法，在一个分配的空间上构造对象
//                        destory(p)方法，对内存p上的对象析构
...
class AlexModelNode : public AlexNode<T, P> {
 public:
  ...
  AlexModelNode(const self_type& other) : AlexNode<T, P>(other), allocator_(other.allocator_), num_children_(other.num_children_) {
  ...
  // Input is the base 2 log of the expansion factor, in order to guarante expanding by a power of 2.
  // Returns the expansion factor.
  // 将 child 数组扩大 2^log2_expansion_factor 倍，返回 2^log2_expansion_factor
...
class AlexDataNode : public AlexNode<T, P> {
 public:
  ...
  // bitmap中的一个元素可以记录 64 个位置是 gap 还是 key
  ...
  // 修改：一个 node 最大 32MB
  static constexpr int kDefaultMaxDataNodeBytes_ = 1 << 25;  // by default, maximum data node size is 16MB
  ...
  // 用于计算 empirical cost
  // 插入操作的移动次数
  unsigned long long num_shifts_ = 0;                 // does not reset after resizing
  // 插入、查找的迭代次数
  unsigned long long num_exp_search_iterations_ = 0;  // does not reset after resizing
  // 执行查找、插入的次数
  ...
  // True if a <= b
  template <class K> forceinline bool key_lessequal(const T& a, const K& b) const {
    return !key_less_(b, a);
  }

  // True if a > b
  template <class K> forceinline bool key_greater(const T& a, const K& b) const {
    return key_less_(b, a);
  }

  // True if a >= b
  template <class K> forceinline bool key_greaterequal(const T& a, const K& b) const {
    return !key_less_(a, b);
  }
  ...
  static double compute_expected_cost_sampling(
    ...
    const static int exact_computation_size_threshold = min_sample_size * sample_size_multiplier * sample_size_multiplier * 2;
  ...
  // 这个 leaf 管理 values 中的所有数据，计算模型参数并填入数据
  void bulk_load(const V values[], int num_keys, const LinearModel<T>* pretrained_model = nullptr, bool train_with_sample = false) {
    ...
    for (int i = 0; i < num_keys; i++) {
      ...
      if (positions_remaining < keys_remaining) {
        // 预测的位置太靠后了，不能按照预测位置填入数据 values[i]
        // 从 pos 位置开始要存剩下的数据了，将 pos 之前都填入 values[i]
        int pos = data_capacity_ - keys_remaining;
        for (int j = last_position + 1; j < pos; j++) {
          ALEX_DATA_NODE_KEY_AT(j) = values[i].first;
          // 修改：存 key 的同时存入 value
          //ALEX_DATA_NODE_PAYLOAD_AT(j) = values[i].second;
        }
        // 从 pos 开始，到数组结尾，填入剩余数据
        for (int j = i; j < num_keys; j++) {
#if ALEX_DATA_NODE_SEP_ARRAYS
      ...
      // 在 values[i] 实际要放入的位置之前都 values[i]
      for (int j = last_position + 1; j < position; j++) {
        ALEX_DATA_NODE_KEY_AT(j) = values[i].first;
        // 12.28 存 key 的同时存入 value
        //ALEX_DATA_NODE_PAYLOAD_AT(j) = values[i].second;
      }
    ...
    // 最后的 key 存在的位置之后填入 kEndSentinel_
  ...
  // node 中的 key_slots_[left] ~ key_slots_[right - 1] 之间的数据创建 data node
  void bulk_load_from_existing(
      const self_type* node, int left, int right, bool keep_left = false, bool keep_right = false,
    ...
    for (; it.cur_idx_ < right && !it.is_end(); it++) {
      ...
      if (positions_remaining < keys_remaining) {
        ...
        for (int j = last_position + 1; j < pos; j++) {
          ...
          //ALEX_DATA_NODE_PAYLOAD_AT(j) = it.payload();
        }
  ...
  static void build_model(const V* values, int num_keys, LinearModel<T>* model, bool use_sampling = false) {
    if (use_sampling) {
  ...
  static void build_model_sampling(const V* values, int num_keys, LinearModel<T>* model, bool verbose = false) {
  ...
  // largest points instead of using a linear regression
  ...
  // 找出最后一个 = key 的 non-gap 位置
  int find_key(const T& key, unsigned long long& num_lookup) {
    num_lookups_++;
    ...
    // 从模型预测位置开始指数搜索，返回的是第一个 > key 的位置，因此要判断 pos 的前一个位置是否是 key
    int pos = exponential_search_upper_bound(predicted_pos, key, num_lookup) - 1;
  ...
  int find_lower(const T& key, unsigned long long& num_lookup) {
    num_lookups_++;
    int predicted_pos = predict_position(key);

    int pos = exponential_search_lower_bound(predicted_pos, key, num_lookup);
  ...
  // Returns position in range [0, data_capacity]
  template <class K>
  inline int exponential_search_upper_bound(int m, const K& key, unsigned long long& num_lookup) {
    ...
    if (key_greater(ALEX_DATA_NODE_KEY_AT(m), key)) {
      ...
      num_lookup++;
#endif
      // m 位置的元素 > key，需要向 m 之前搜索
      int size = m;
      // 指数搜索找到第一个 < key的位置，作为之后二分搜索的下界
    ...
    return binary_search_upper_bound(l, r, key, num_lookup);
  }
  ...
  // 在 [l,r) 之间搜索第一个 > key 的位置，但可以返回 r。如果返回 r 说明在区间 [l,r) 之间没有 key
  template <class K>
  inline int binary_search_upper_bound(int l, int r, const K& key, unsigned long long& num_lookup) const {
    while (l < r) {
#ifdef TIME
      num_lookup++;
#endif
  ...
  // Returns position in range [0, data_capacity]
  template <class K>
  inline int exponential_search_lower_bound(int m, const K& key, unsigned long long& num_lookup) {
    ...
    return binary_search_lower_bound(l, r, key, num_lookup);
  }
  ...
  // https://stackoverflow.com/questions/6443569/implementation-of-c-lower-bound
  // Returns position in range [l, r]
  // 返回 r 说明区间 [l,r) 中不存在 >= key 的元素
  template <class K>
  inline int binary_search_lower_bound(int l, int r, const K& key, unsigned long long& num_lookup) const {
    while (l < r) {
#ifdef TIME
      num_lookup++;
#endif
  ...
  std::pair<int, int> insert(const T& key, const P& payload) {
    ...
    // catastrophe：检查 leaf 的 shifts_per_insert 和 expected_avg_shifts_ 是否过大，过大则将 leaf 分裂。每执行 64 次插入检查一次
    // 修改：不进行 expand_root 的操作，也要删除 catastrophic_cost 的判断
    ...
    if (insertion_position < data_capacity_ && !check_exists(insertion_position)) {
      // 这个位置为空，直接插入
#ifdef PRINT_PROCESS
        std::cout << "insert_element_at" << std::endl;
#endif
      ...
      // 这个位置非空
#ifdef PRINT_PROCESS
        std::cout << "insert_using_shifts" << std::endl;
#endif
  ...
  void resize(double target_density, bool force_retrain = false, bool keep_left = false, bool keep_right = false) {
...
#endif
    if (new_bitmap == nullptr || new_key_slots == nullptr || new_payload_slots == nullptr) {
    ...
    const_iterator_type it(this, 0);
#ifdef PRINT_PROCESS
    std::cout << "resize, insert again" << std::endl;
#endif
    for (; it.cur_idx_ < data_capacity_ && !it.is_end(); it++) {
      ...
      if (positions_remaining < keys_remaining) {
        ...
        for (int j = last_position + 1; j < pos; j++) {
          ...
          // 修改：每个位置都填入 payload
          new_payload_slots[j] = it.payload();
#else
  ...
  int insert_using_shifts(const T& key, P payload, int pos) {
    ...
    set_bit(gap_pos);
    if (gap_pos >= pos) {
      // 位于 [pos, gap_pos - 1] 的元素都往后移动一个位置，空出 pos
      ...
      // 位于 [gap_pos + 1, pos - 1] 的元素都往前移动一个位置，空出 pos - 1
  ...
  int closest_gap(int pos) const {
    ...
    if (bitmap_[bitmap_pos] == static_cast<uint64_t>(-1) ||
      ...
      // no gaps in this block of 64 positions, start searching in adjacent sblocks
  ...
  int erase_one(const T& key, unsigned long long& num_lookup, unsigned long long& num_fill) {
    int pos = find_lower(key, num_lookup);

    if (pos == data_capacity_ || !key_equal(ALEX_DATA_NODE_KEY_AT(pos), key))
      return 0;

    // Erase key at pos
    erase_one_at(pos, num_fill);
    return 1;
  }
  ...
  void erase_one_at(int pos, unsigned long long& num_fill) {
    T next_key;
    if (pos == data_capacity_ - 1) {
      next_key = kEndSentinel_;
    } else {
