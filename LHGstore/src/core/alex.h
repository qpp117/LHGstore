...
// node's parent.
#if ALEX_SAFE_LOOKUP
  forceinline data_node_type* get_leaf(T key, std::vector<TraversalNode>* traversal_path = nullptr) const {
    if (traversal_path) {
...
// 记录走向下一层次数
#if ALEX_SAFE_LOOKUP
  forceinline data_node_type* get_leaf(T key, unsigned long long& num_get_child, 
...
#ifdef PRINT_PROCESS
      std::cout << "Recursively bulk load, build data node directly" << std::endl;
#endif
      data_node->bulk_load(values, num_keys, data_node_model, params_.approximate_model_computation);
#ifdef PRINT_PROCESS
      std::cout << "Recursively bulk load, build data node directly finish" << std::endl;
#endif
...
#ifdef PRINT_PROCESS
    std::cout << "Recursively bulk load, build fanout tree finish" << std::endl;
#endif
 ...
 public:
  ...
  // 记录搜索次数和查询下一层节点次数
  P* get_payload(const T& key, unsigned long long& num_lookup, unsigned long long& num_get_child) const {
    stats_.num_lookups++;
    data_node_type* leaf = get_leaf(key, num_get_child);
    int idx = leaf->find_key(key, num_lookup);
    if (idx < 0) {
      return nullptr;
    } else {
  ...
  // 记录搜索次数
  P* get_payload(const T& key, unsigned long long& num_lookup) const {
    stats_.num_lookups++;
    data_node_type* leaf = get_leaf(key);
    int idx = leaf->find_key(key, num_lookup);
    if (idx < 0) {
 ...
 public:
  ...
  std::pair<Iterator, bool> insert(const T& key, const P& payload) {
    ...
    // 超出了现有的 key space，判断要不要 expand root
    // 修改：expand_root 会导致 bug
...
//    }
...
#ifdef PRINT_PROCESS
                std::cout << "insert edge in secondary index failed, split downward" << std::endl;
#endif
              parent = split_downwards(parent, bucketID, fanout_tree_depth, used_fanout_tree_nodes, reuse_model);
            } else {
#ifdef PRINT_PROCESS
                std::cout << "insert edge in secondary index failed, split sideway" << std::endl;
#endif
...
#ifdef PRINT_PROCESS
        std::cout << "insert edge in secondary index failed, try again" << std::endl;
#endif
        ret = leaf->insert(key, payload);
#ifdef PRINT_PROCESS
        std::cout << "insert edge in secondary index again, return value = " << ret.first << std::endl;
#endif
        fail = ret.first;
 ...
 ////private
  ...
  // 扩展 root 的条件（同时满足）
  // (1) root 不是 leaf
  // (2) 已经有至少 kMinOutOfDomainKeys = 5 个插入的 key 超出了 key space
  // (3-1) 有至少 kMaxOutOfDomainKeys = 1000 个插入的 key 超出了 key space 或；
  // (3-2) 有 kOutOfDomainToleranceFactor（= 2） * (stats_.num_keys / istats_.num_keys_at_last_right_domain_resize - 1) 个插入的 key 超出了 key space
  ...
  // a new root node.
  // 扩展 index 能够存储的最大/最小key
  void expand_root(T key, bool expand_left) {
      //std::cout << "expand root, to_left = " << expand_left << ", key = " << key << std::endl;
    ...
    if (expand_left) {
      // 增加的 domain size = 原来的 key_domain_min_ - 当前最小 key
      auto key_difference = static_cast<double>(istats_.key_domain_min_ - std::min(key, get_min_key()));
      // root 的 children 扩大多少倍
      expansion_factor = pow_2_round_up(static_cast<int>(std::ceil((key_difference + domain_size) / domain_size)));
      // Check for overflow.
      // 比较 istats_.key_domain_max_ 和 expansion_factor * domain_size 的大小， /2 之后再比较避免溢出
      T half_expandable_domain = istats_.key_domain_max_ / 2 - std::numeric_limits<T>::lowest() / 2;
      // 扩展之后，root->num_children_ 个 child 指针指向一个 domain_size 范围的 new data node
      // 因此最大的可扩展是 expansion_factor * domain_size
      T half_expanded_domain_size = expansion_factor / 2 * domain_size;
      // new_domain_min：扩展之后能够达到的最小 domain
      if (half_expandable_domain < half_expanded_domain_size) {
        // 扩展之后的 new_domain_min 小于当前数据类型的最小值了
    ...
    // [new_nodes_start, new_nodes_end) 之间的指针需要指向新的 data node，其它的指针和原来一样
    int new_nodes_start;  // index of first pointer to a new node
    int new_nodes_end;    // exclusive
    if (static_cast<size_t>(root->num_children_) * expansion_factor <= static_cast<size_t>(derived_params_.max_fanout)) {
      // 不需要分裂 root
      // Expand root node
      ...
      // 修改：初始化为 null，expand 时需要判断
      for (int i = 0; i < new_num_children; i++) {
          new_children[i] = nullptr;
      }
      // copy_start 及之后的指针复制原来的
      int copy_start;
      if (expand_left) {
      ...
      auto new_root = new (model_node_allocator().allocate(1)) model_node_type(static_cast<short>(root->level_ - 1), allocator_);
    ...
    // 扩展后的 new_domain_min, new_domain_max 超过了当前数据类型的最小、最大值，有几个 child 指针用不上了
    // 修改：会导致 expand model node 时的 bug
    if (expand_left) {
    ...
    // 将原来的第一个 child（向左扩展）或最后一个 child（向右扩展）管理的数据分成多个 data node
    // 每 n 个 child 指向一个 new data node
    ...
    if (expand_left) {
      ...
      data_node_type* new_node = nullptr;
      for (int i = new_nodes_end; i > new_nodes_start; i -= n) {
        ...
        // 为 outermost_node 的 [left_boundary, right_boundary) 部分数据创建一个 data node
        int right_boundary = left_boundary;
        if (i - n <= in_bounds_new_nodes_start) {
          // 这是最后一个 new data node，直接用剩下的所有数据创建
          left_boundary = 0;
        } else {
          // 一个 domain_size 范围的数据创建一个 new data node
          left_boundary_value -= domain_size;
          left_boundary = outermost_node->lower_bound(left_boundary_value);
        }
        new_node = bulk_load_leaf_node_from_existing(outermost_node, left_boundary, right_boundary, true);
        ...
        next = new_node;
        // n 个 children_ 中的指针指向一个新的 data node
    ...
    // 删除 outermost_node 中分离出去的数据
    if (expand_left) {
      outermost_node->erase_range(new_domain_min, istats_.key_domain_min_);
      auto last_new_leaf = static_cast<data_node_type*>(root->children_[new_nodes_end - 1]);
      ...
      last_new_leaf->next_leaf_ = outermost_node;
        ...
        auto first_new_leaf = static_cast<data_node_type*>(root->children_[new_nodes_start]);
  ...
  void split_sideways(model_node_type* parent, int bucketID, int fanout_tree_depth,
...
#ifdef PRINT_PROCESS
        std::cout << "split sideways, expand parent" << std::endl;
#endif
      stats_.num_model_node_expansions++;
    ...
    int start_bucketID = bucketID - (bucketID % repeats);  // first bucket with same child
#ifdef PRINT_PROCESS
    std::cout << "split sideways, expand parent finish" << std::endl;
#endif
    if (used_fanout_tree_nodes.empty()) {
      assert(fanout_tree_depth == 1);
...
#ifdef PRINT_PROCESS
    std::cout << "split sideways, create two new node finish" << std::endl;
#endif
    delete_node(leaf);
#ifdef PRINT_PROCESS
    std::cout << "delete old leaf finish" << std::endl;
#endif
    stats_.num_data_nodes--;
  }
 ...
 public:
  ...
  // Erases the left-most key with the given key value
  // 记录比较次数、填充次数、走向下一层节点次数
  int erase_one(const T& key, unsigned long long& num_lookup, unsigned long long& num_fill, unsigned long long& num_get_child) {
    data_node_type* leaf = get_leaf(key, num_get_child);
    int num_erased = leaf->erase_one(key, num_lookup, num_fill);
    stats_.num_keys -= num_erased;
    if (leaf->num_keys_ == 0) {
      merge(leaf, key);
    }
