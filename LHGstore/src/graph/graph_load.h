...
bool LoadDiredtedGraph(const std::string file_path, std::vector<edge>& all_edge, 
...
// 不检查重边
bool LoadUndirectedGraphNoCheck(const std::string file_path, std::vector<edge>& all_edge, std::vector<vertex>& all_vertex, 
    std::unordered_map<vertex, int>& vertex_deg, size& total_num_edges, size load_num_edges) {
    std::ifstream f(file_path.c_str(), std::ios::in);
    if (!f.is_open()) {
        return false;
    }
...
std::string* ReadOptionFromFile(int& argc, std::string file_name = "option.txt") {
    argc = 0;
